...
class MerchantSignatureTestingApi
{
    ...
    {
        return $this->generateSignAsyncWithHttpInfo($private_key, $body)->then(function ($response) {
            return $response[0];
        });
    }
    /**
    ...
    protected function generateSignRequest($private_key, $body = null)
    {
        ...
        if ($private_key === null || \is_array($private_key) && \count($private_key) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $private_key when calling generateSign');
        }
        ...
        // path params
        if ($private_key !== null) {
            $resourcePath = \str_replace('{' . 'privateKey' . '}', ObjectSerializer::toPathValue($private_key), $resourcePath);
        }
        // body params
        $_tempBody = null;
        if (isset($body)) {
        ...
        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(['text/plain']);
        } else {
            $headers = $this->headerSelector->selectHeaders(['text/plain'], ['application/json']);
        }
        // for model (json/xml)
        if (isset($_tempBody)) {
