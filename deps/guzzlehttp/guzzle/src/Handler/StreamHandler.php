...
namespace RelayPayDeps\GuzzleHttp\Handler;

use RelayPayDeps\GuzzleHttp\Exception\ConnectException;
use RelayPayDeps\GuzzleHttp\Exception\RequestException;
use RelayPayDeps\GuzzleHttp\Promise\FulfilledPromise;
use RelayPayDeps\GuzzleHttp\Promise\PromiseInterface;
use RelayPayDeps\GuzzleHttp\Psr7;
use RelayPayDeps\GuzzleHttp\TransferStats;
use RelayPayDeps\GuzzleHttp\Utils;
use RelayPayDeps\Psr\Http\Message\RequestInterface;
use RelayPayDeps\Psr\Http\Message\ResponseInterface;
use RelayPayDeps\Psr\Http\Message\StreamInterface;
/**
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \RelayPayDeps\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \RelayPayDeps\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \RelayPayDeps\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \RelayPayDeps\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\RelayPayDeps\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \RelayPayDeps\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \RelayPayDeps\GuzzleHttp\debug_resource($value);
