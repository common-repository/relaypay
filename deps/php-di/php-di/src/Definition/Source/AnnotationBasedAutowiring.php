...
use RelayPayDeps\DI\Annotation\Inject;
use RelayPayDeps\DI\Annotation\Injectable;
use RelayPayDeps\DI\Definition\Exception\InvalidAnnotation;
use RelayPayDeps\DI\Definition\ObjectDefinition;
use RelayPayDeps\DI\Definition\ObjectDefinition\MethodInjection;
use RelayPayDeps\DI\Definition\ObjectDefinition\PropertyInjection;
use RelayPayDeps\DI\Definition\Reference;
use RelayPayDeps\Doctrine\Common\Annotations\AnnotationRegistry;
use RelayPayDeps\Doctrine\Common\Annotations\Reader;
use RelayPayDeps\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use RelayPayDeps\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'RelayPayDeps\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'RelayPayDeps\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('RelayPayDeps\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'RelayPayDeps\\DI\\Annotation\\Injectable');
